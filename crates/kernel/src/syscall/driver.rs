...
pub(crate) fn handle(number: u64, arg0: u64, arg1: u64, _arg2: u64) -> u64 {
    ...
    // KazuOS driver modules
            // NOTE: driver load/unload is deliberately open to PrivilegeLevel::User, which
            // means any process can gain Driver capabilities (ioport/DMA/IRQ/PCI BAR) by
            // loading a .kdm. That is a known gap, not an oversight: there is no privileged
            // session concept yet, so gating this at Driver would leave `/bin/drivers` — the
            // only runtime driver management there is — unable to do anything. Closing it
            ...
            SYS_DRIVER_LOAD => {
                ...
                match crate::memory::uaccess::read_str(arg0, arg1) {
                    Some(path) => crate::driver_module::load(&path),
                    None => u64::MAX,
                }
            }
            SYS_DRIVER_UNLOAD => {
                if crate::driver_module::unload(arg0 as u32) { 0 } else { u64::MAX }
            }
            SYS_DRIVER_LIST => crate::driver_module::list(arg0, arg1),
            SYS_DRIVER_INFO => crate::driver_module::info(arg0 as u32, arg1),
