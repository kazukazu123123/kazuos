...
// Pipe buffers are shared across CPUs (a writer on one CPU, a reader on another),
// so every access is serialized under the reentrant thread lock — otherwise the
// concurrent Vec extend/drain corrupts the buffer and bytes are lost. The thread
// lock is reused (not a private one) because read_raw runs from inside
// notify_pipe_readers, which already holds it; the reentrant guard makes that safe.
fn with_lock<F: FnOnce() -> R, R>(f: F) -> R {
    crate::task::thread::with_threads_lock(f)
}

fn pipes() -> &'static mut Vec<Option<PipeState>> {
    unsafe { &mut *PIPES.0.get() }
}
