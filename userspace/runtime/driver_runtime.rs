...
/// Metadata returned by kdm_info().
#[repr(C)]
pub struct KdmInfo {
    pub name:    &'static str,
    /// Null-terminated list of driver names this driver depends on.
    pub depends: &'static [&'static str],
}

/// Driver entry point generated by driver_runtime. Calls the driver's four
/// lifecycle functions in order:
///   kdm_info()  — metadata query (not called here, used by build tooling)
///   kdm_init()  — hardware initialisation, returns false on failure
///   kdm_run()   — main event loop, returns when a stop signal is received
///   kdm_exit()  — cleanup
#[unsafe(no_mangle)]
pub extern "C" fn _start(_argc: u64, _argv: u64) -> ! {
    // Enable signal catching so driver_module::unload() can reach us.
...
/// Load a KazuOS driver module by path. Returns driver_id on success, u64::MAX on failure.
pub fn sys_driver_load(path: &[u8]) -> u64 {
    let r: u64;
    unsafe {
        core::arch::asm!(
            "int 0x80",
...
/// Unload a KazuOS driver module by id. Returns true on success.
pub fn sys_driver_unload(id: u64) -> bool {
    let r: u64;
    unsafe {
        core::arch::asm!(
            "int 0x80",
...
/// Fill `buf` with driver list entries (48 bytes each). Returns entry count.
pub fn sys_driver_list(buf: &mut [u8]) -> u64 {
    let r: u64;
    unsafe {
        core::arch::asm!(
            "int 0x80",
...
/// Fill `buf` (48 bytes) with info for driver `id`. Returns true on success.
pub fn sys_driver_info(id: u64, buf: &mut [u8]) -> bool {
    let r: u64;
    unsafe {
        core::arch::asm!(
            "int 0x80",
            inlateout("rax") SYS_DRIVER_INFO => r,
            in("rdi") id,
